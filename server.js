...
app.delete('/pacientes/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'ALTER TABLE turnos ADD CONSTRAINT turnos_id_paciente FOREIGN KEY (id_paciente) REFERENCES pacientes(id);');
    ...
    res.status(500).send(`Hubo un error al eliminar el paciente: ${error.message}`);
  }
});
...
app.post('/activar-restriccion-externa', async (req, res) => {
  try {
      // Reactivar la restricción de clave externa en la columna 'id_paciente' de la tabla 'turnos'
      await pool.query('ALTER TABLE turnos ADD CONSTRAINT IF NOT EXISTS turnos_id_paciente FOREIGN KEY (id_paciente) REFERENCES pacientes(id);');
      ...
      res.status(500).json({ error: 'Error al activar restricción externa.', detalle: error.message });
    }
});